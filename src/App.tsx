...
function App() {
  ...
  const triggerSave = useCallback(() => {
    ...
    logger.debug('[App] Déclenchement sauvegarde (via raccourci)...');
    
    // << MODIFICATION: Copier dans le presse-papiers au lieu de logguer tout >>
    try {
        const blocksJson = JSON.stringify(editorBlocks, null, 2);
        navigator.clipboard.writeText(blocksJson)
            .then(() => {
                logger.debug('[App] État des blocs copié dans le presse-papiers.');
                alert('État des blocs copié dans le presse-papiers ! Collez-le dans un éditeur de texte.');
            })
            .catch(err => {
                logger.error('[App] Erreur lors de la copie dans le presse-papiers:', err);
                // Fallback: tenter de logguer une partie si la copie échoue
                console.log("--- État des blocs (Copie échouée, log partiel) ---");
                console.log(editorBlocks); 
            });
    } catch (error) {
        logger.error('[App] Erreur lors de la conversion des blocs en JSON:', error);
    }
    
    if (editorBlocks.length === 0) {
        logger.warn('[App] Tentative de sauvegarde avec des blocs vides (après copie). Annulation.');
        return; 
    }
    const markdownToSave = blocksToMarkdown(editorBlocks);
    setEditorMarkdown(markdownToSave); // Optionnel: mettre à jour l'état local
    console.log("--- Sauvegarde profonde (Markdown généré via Ctrl+S) ---");
    console.log(markdownToSave);
    setHasUnsavedChanges(false);
    // TODO: Remplacer le console.log par l'appel à l'API/localStorage
    // Optionnel: Ajouter un petit feedback visuel (ex: toast "Sauvegardé !")
