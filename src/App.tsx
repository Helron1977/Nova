...
function App() {
  ...
  const triggerPdfPrint = useCallback((htmlContent: string) => {
    const printStyles = `
      @media print {
        @page {
          margin: 15mm;
        ...
        h1 { font-size: 20pt; margin-top: 1.2em; margin-bottom: 0.5em; page-break-after: avoid; break-after: avoid; }
        h2 { font-size: 16pt; margin-top: 1.2em; margin-bottom: 0.5em; page-break-after: avoid; break-after: avoid; }
        h3 { font-size: 14pt; margin-top: 1.2em; margin-bottom: 0.5em; page-break-after: avoid; break-after: avoid; }
        h4, h5, h6 { font-size: 12pt; margin-top: 1.2em; margin-bottom: 0.5em; page-break-after: avoid; break-after: avoid; }
        p, ul, ol { margin-bottom: 0.8em; }
        
        /* Gestion des sauts de page (ne pas couper au milieu) */
        p, li, blockquote, pre, table, tr, img, .mermaid-diagram-container, .nova-block-summary-exported { 
        ...
        /* Palette de couleurs */
        .custom-html-palette-container { display: flex; flex-wrap: wrap; padding: 10px; margin-bottom: 1.5em; border: 1px solid #eee; border-radius: 4px; background-color: #f9f9f9; page-break-inside: avoid; break-inside: avoid; }
        .palette-color-item { width: 120px; margin: 8px; border: 1px solid #ddd; border-radius: 4px; text-align: center; font-family: 'Consolas', 'Menlo', monospace; font-size: 12px; background-color: #fff; padding-bottom: 5px; overflow: hidden; page-break-inside: avoid; break-inside: avoid; }
        ...
        /* Classe utilitaire pour forcer un saut de page manuel (ex: via HTML brut) */
        .page-break {
    ...
    if (doc) {
      ...
      iframe.onload = () => {
        ...
        setTimeout(() => {
          ...
          logger.debug('[App] Fenêtre d\'impression pour PDF déclenchée.');
          setHasUnsavedChanges(false);
        }, 1000);
      };
    } else {
      logger.error('[App] Impossible d\'accéder au document de l\'iframe pour l\'impression PDF.');
      if (document.body.contains(iframe)) {
        document.body.removeChild(iframe);
      }
    }
  ...
  const executeExport = useCallback(async (formatChoice: string) => {
    ...
    try {
      ...
      if (formatChoice === 'html' || formatChoice === 'pdf') {
        logger.debug('[App] EXPORT: Format HTML/PDF choisi. Rendu des diagrammes Mermaid...');
...
<head>
  ...
  <style>
    ...
    .palette-color-item-display { width: 100%; height: 80px; border-bottom: 1px solid #eee; }
    .palette-color-item-name { padding: 5px 3px; font-weight: bold; color: #333; word-wrap: break-word; }
    .palette-color-item-hex { color: #555; font-size: 11px; }
...
${finalHtmlContent}
          ...
          break;
        case 'pdf':
          logger.debug('[App] EXPORT: Format PDF chosen.');
          if (finalHtmlContent) {
            triggerPdfPrint(finalHtmlContent);
          } else {
            logger.error('[App] EXPORT: HTML content is null for PDF export. This should not happen.');
          }
          break;
        default:
  ...
  }, [blocksState.blocks, downloadHtmlFile, triggerPdfPrint, setHasUnsavedChanges, renderMermaidDiagramsInMarkdown, prepareMarkdownContent]);
